...
def set_detector(graph_storage):
    original_upsert_edge = graph_storage.upsert_edge

    async def detector(source_id, target_id, edge_data):
        new_relation = edge_data.get("description", "")
        existing = await graph_storage.get_edge(source_id, target_id)

        if existing:
            existing_relation = existing.get("description", "")
            similarity = cosine_sim(await embed(existing_relation), await embed(new_relation))
            if similarity > SIMILARITY_THRESHOLD:
                verdict = await llm_judge(existing_relation, new_relation)
                print(f"[detector] {source_id} <-> {target_id}: {verdict} (similarity={similarity:.2f})")

        return await original_upsert_edge(source_id, target_id, edge_data)

    graph_storage.upsert_edge = detector
...
async def main():
    ...
    set_detector(rag.chunk_entity_relation_graph)
